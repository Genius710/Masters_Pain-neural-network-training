...
mat_fname = pjoin('C:\OneDrive - Kaunas University of Technology\MAGISTRINIS\EqualisedDataMatrixFeaturesV5.mat')
...
mat_contents = sio.loadmat(mat_fname,struct_as_record=1)
DataMatrix = mat_contents['EqualisedDataMatrixFeaturesV']
# DataMatrix = DataMatrix[0,:]
...
# model = tf.keras.Sequential(
#         [
#             tf.keras.layers.Input(shape=[multi,VACL]),
            
#             tf.keras.layers.LSTM(VACL*multi),
#             # tf.keras.layers.Dense(VACL*multi, activation ='relu'),
#             # tf.keras.layers.Dense(VACL*multi, activation ='relu'),
#             # tf.keras.layers.Dense(VACL*multi, activation ='relu'),
...
for iii in range(1,10):
    ...
    model.save(pjoin('Feature_model_68_MLP_multi_5_v1/',str(iii*100)))
