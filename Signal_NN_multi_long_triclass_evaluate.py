...
path = 'RawSignalLong1_MLP_v1_triclass/'
...
path = 'RawSignalLong12_CNN_v1_triclass/'
...
path = 'RawSignalLong12_MLP_v1_triclass/'
model = tf.keras.models.load_model(pjoin(path,'500')) 
...
path = 'RawSignalLong12_LSTM_v1_triclass/'
model = tf.keras.models.load_model(pjoin(path,'500')) 
model.compile('adamax', loss='categorical_crossentropy',metrics=[tf.keras.metrics.CategoricalAccuracy()])
np.savetxt(pjoin(path,'LSTM_Long12_results.txt'),model.predict(InputLongV12))
